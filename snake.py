...
#Set Images (use simple rects so just create their coordinates)
...
while running:
    ...
    #Update the x,y position of snakes head and make a new coordinate
    head_x += snake_dx
    head_y += snake_dy
    head_coord = (head_x, head_y, SNAKE_SIZE, SNAKE_SIZE)
    ...
    if head_rect.left < 0 or head_rect.right > WINDOW_WIDTH or head_rect.top < 0 or head_rect.bottom > WINDOW_HEIGHT or head_coord in body_coords:
        ...
        #Pause Game until the player presses a key, then reset the game
        is_paused = True
        while is_paused:
            pygame.time.wait(2000)
            for event in pygame.event.get():
                #Player wants to play again reset game
                if event.type == pygame.KEYDOWN:
                    score = 0 
